...
def qwen3_5_decoder_layer_forward(
    ...
    **kwargs,
    ...
    if layer_type is None:
        # FSDP2 may replace the module class with a dynamically generated
        # wrapper that retains child modules but not plain instance attributes.
        if getattr(self, "linear_attn", None) is not None:
            layer_type = "linear_attention"
        elif getattr(self, "self_attn", None) is not None:
            layer_type = "full_attention"
        else:
            raise AttributeError("Qwen3.5 decoder layer has no recognizable attention module")
